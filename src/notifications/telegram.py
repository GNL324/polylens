...
def format_prop_alert(opportunity: dict[str, Any], bankroll: float | None = None) -> str:
    ...
    lines = [
        ...
        "",
        "Over:",
        f"{opportunity.get('over_book')} ({opportunity.get('over_source') or 'odds-api'}) {opportunity.get('over_odds')}",
        "",
        "Under:",
        f"{opportunity.get('under_book')} ({opportunity.get('under_source') or 'odds-api'}) {opportunity.get('under_odds')}",
        "",
        f"ROI: {roi * 100:.2f}%",
